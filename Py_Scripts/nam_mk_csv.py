...
'''
Named Make CSV Python Program
Only produces type N accounts with no partners
'''

def csv_file(email_list):
    with open('../nameed_email.csv', 'w') as myfile:
        wr = csv.writer(myfile, dialect='excel')
        wr.writerow(email_list)
    
def m_csv():
    locations = ["lv", "az", "nm"]
    e_csv = open('../named_email.csv', 'w+')
    countr = 0
    for l in locations:
        with open('../Contacts/'+l+'_contacts.json') as j_file:
            ...
            for e in payload:
                if countr == 0:
                    ...
                    countr+=1
                if len(e['email']) >1:
                    if e['type'] != "T" and e['category'] != "partner":
                        first = e['name'].split(" ")
